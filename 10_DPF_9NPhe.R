...
Data.10 <- subset(Data, DPF == 10)



#Visualize Data
ggplot(Data.10, aes(x = Concentration_μM,y = BPM)) + 
  geom_boxplot(aes(group = Concentration_μM))

#Visualize Normal Distribution 
ggplot(Data.10, aes(x = BPS)) +
  geom_histogram() + 
  xlim(1, 2) + 
  facet_wrap(~Concentration_μM, ncol = 1)

#Shapiro Wilk Test for Normal Distrubution 
ShW.0 <- subset(Data.10, Concentration_μM == 0)
shapiro.test(ShW.0$BPM)

ShW.1 <- subset(Data.10, Concentration_μM == 1)
shapiro.test(ShW.1$BPM)

ShW.5 <- subset(Data.10, Concentration_μM == 5)
shapiro.test(ShW.5$BPM)

ShW.20 <- subset(Data.10, Concentration_μM == 20)
shapiro.test(ShW.20$BPM)

ShW.50 <- subset(Data.10, Concentration_μM == 50)
...
bartlett.test(BPM ~ Concentration_μM, data = Data.10)

#Kruskal-Wallis Test 
KW.test <- kruskal.test(Data.10$BPM, Data.10$Concentration_μM)
print(KW.test)

#Post hoc = Dunnet Test 
DunnettTest(x = Data.10$BPM, g = Data.10$Concentration_μM)
DunnettTest(x = Data.10$BPM, g = Data.10$Concentration_μM) %>% plot()
